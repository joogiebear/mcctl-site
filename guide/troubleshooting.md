...
- **A scheduled task did not run:** confirm the task is enabled, review its run history, and keep the Mac awake and your user session logged in for the scheduled time.
...
- **A CLI backup is missing from the panel:** the visible history refreshes every
